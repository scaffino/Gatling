...
def parse_log_line(line):
    ...
    Returns:
        tuple: (log_datetime, transaction_hash, tx_timestamp_ms) or None
               if not a finalization line
    """
    # Pattern to match finalization lines
    # Example: "Transaction a81454ee...947c8e (timestamp: 1760623008123 ms)
    # is now final in block"
    finalization_pattern = (
        r'Transaction ([a-f0-9]+) \(timestamp: (\d+) ms\) '
        r'is now final in block'
    )
    ...
    if timestamp_match and finalization_match:
        ...
        log_datetime = datetime.strptime(
            log_timestamp_str, '%Y-%m-%dT%H:%M:%S.%f'
        )
...
def main():
    ...
    # Track transaction hashes we've already processed
    ...
    print("=" * 95)
    print("{:<66} {:<15} {:<15}".format(
        'Transaction Hash', 'Latency (ms)', 'Latency (s)'
    ))
    print("-" * 95)
    ...
    for line in lines:
        ...
        if result:
            ...
            log_timestamp_ms = (
                calendar.timegm(log_datetime.timetuple()) * 1000 +
                log_datetime.microsecond / 1000.0
            )
            ...
            # Output the result
            print("{:<66} {:<15.3f} {:<15.2f}".format(
                tx_hash, latency_ms, latency_s
            ))
            finalization_count += 1

    print("-" * 95)
    print("Total finalized transactions found: {0}".format(
        finalization_count
    ))
    if duplicate_count > 0:
        print("Duplicate finalizations skipped: {0}".format(
            duplicate_count
        ))
    ...
    if finalization_count > 0:
        ...
        print("Average latency: {0:.3f} ms ({1:.2f} s)".format(
    ...
    if finalization_count == 0:
        ...
        print('  "Transaction <hash> (timestamp: <unix_timestamp_ms> ms) '
              'is now final in block..."')
