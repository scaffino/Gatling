...
for host in "${hosts[@]}"; do
  ...
  echo "Downloading metrics from ${host} (${host_id})..."
  host_metrics_dir="${LOCAL_METRICS_DIR}/${host_id}"
  mkdir -p "${host_metrics_dir}"

  # Download all K directories (K*/...) if present. Use per-host namespace to avoid collisions.
  ssh "${SSH_OPTS[@]}" "${host}" "ls -d ${REMOTE_METRICS_DIR}/K* 2>/dev/null" | while read -r kdir; do
    [[ -n "${kdir}" ]] || continue
    kname="$(basename "${kdir}")" # e.g., K35
    echo "  Downloading ${kname}/ ..."
    mkdir -p "${host_metrics_dir}/${kname}"
    scp -r "${SSH_OPTS[@]}" "${host}:${kdir}/" "${host_metrics_dir}/" || {
      echo "  Warning: Failed to download metrics ${kname} from ${host}" >&2
    }
  done
done
