...
if [[ "${SKIP_REMOTE_DEPLOY:-0}" == "1" ]]; then
    echo ""
    echo "SKIP_REMOTE_DEPLOY=1 – skipping remote deployment. Configs available in ${OUTPUT_DIR_ABS}"
else
...
if [[ "${DEPLOYED_REMOTELY}" -eq 1 ]]; then
    ...
    echo "  - Skipped remote deployment (SKIP_REMOTE_DEPLOY=1)"
    echo ""
    echo "Configs available locally at: ${OUTPUT_DIR_ABS}"
fi
echo ""
